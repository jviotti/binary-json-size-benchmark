...
const REGEX_SYMBOLS: RegExp = new RegExp([
  // eslint-disable-next-line max-len
  '([\\0-\\u02FF\\u0370-\\u1DBF\\u1E00-\\u20CF\\u2100-\\uD7FF\\uDC00-\\uFE1F\\uFE30-\\uFFFF]',
  '[\\uD800-\\uDBFF][\\uDC00-\\uDFFF]',
  // eslint-disable-next-line max-len
...
export const unicodeLength = (input: string): number => {
  const stripped: string = input
    .replace(ANSI_REGEX, '')
    .replace(REGEX_SYMBOLS, (_match: string, element: string) => {
      return element
    })
  ...
  while (counter < stripped.length) {
    result += 1
    const value = stripped.charCodeAt(counter)
    counter += 1
    if (value >= 0xD800 &&
      value <= 0xDBFF &&
